...
async function selectPlan(businessId, input) {
    const plan = await prisma_1.prisma.plan.findFirst({ where: { name: String(input.planName || '').trim(), isActive: true } });
    if (!plan)
    ...
    const paymentPhone = input.paymentPhone ? (0, phone_1.normalizeTzPhone)(input.paymentPhone) : null;
...
async function confirmPayment(businessId, input) {
    const planName = String(input.planName || input.planId || '').trim();
    if (!planName)
        throw new httpError_1.HttpError(400, 'INVALID_PLAN', 'planName is required');
    const plan = await prisma_1.prisma.plan.findFirst({ where: { name: planName, isActive: true } });
    if (!plan)
        throw new httpError_1.HttpError(400, 'INVALID_PLAN', 'Invalid plan');
    const amount = Number(input.amount ?? 0);
    if (!Number.isFinite(amount) || amount < 0)
        throw new httpError_1.HttpError(400, 'INVALID_AMOUNT', 'Invalid amount');
    if (amount !== Number(plan.price))
        throw new httpError_1.HttpError(400, 'INVALID_AMOUNT', `Amount must be ${Number(plan.price)} for plan ${plan.name}`);
    const reference = input.reference != null ? String(input.reference).trim() : '';
    if (amount > 0 && !reference)
        throw new httpError_1.HttpError(400, 'MISSING_REFERENCE', 'Payment reference is required');
    const months = 1;
    const discountPercent = 0;
    const durationDays = SUBSCRIPTION_DURATION_DAYS;
    const startedAt = new Date();
    const endsAt = new Date(startedAt.getTime() + durationDays * 24 * 60 * 60 * 1000);
    const paymentPhone = input.phoneNumber ? (0, phone_1.normalizeTzPhone)(input.phoneNumber) : null;
