...
import { hashToken, randomId, signAccessToken, signRefreshToken } from '../config/jwt';

type UserRole = 'ADMIN' | 'MANAGER' | 'CASHIER' | 'ATTENDANT' | 'STAFF';
const UserRole = {
  ADMIN: 'ADMIN',
  MANAGER: 'MANAGER',
  CASHIER: 'CASHIER',
...
const toUserRole = (role: string): UserRole => {
  ...
  if (r === 'CASHIER') return UserRole.CASHIER;
  if (r === 'ATTENDANT') return UserRole.ATTENDANT;
...
export async function signup(input: {
  ...
  const normalizedPhone = normalizeTzPhone(input.ownerPhone);
  if (!normalizedPhone) throw new HttpError(400, 'INVALID_PHONE', 'Phone number is invalid');

  const existingPhone = await prisma.user.findFirst({
    where: { phone: normalizedPhone }
  });
  if (existingPhone) throw new HttpError(409, 'PHONE_EXISTS', 'Phone number already exists');

  const plan = await prisma.plan.findFirst({ where: { name: input.planName, isActive: true } });
  if (!plan) throw new HttpError(400, 'INVALID_PLAN', 'Invalid plan');

  const module = await prisma.module.findFirst({ where: { key: input.moduleKey, isActive: true } });
  if (!module) throw new HttpError(400, 'INVALID_MODULE', 'Invalid module');
  ...
  const paymentPhone = input.subscription?.paymentPhone ? normalizeTzPhone(input.subscription.paymentPhone) : null;
  if (status === SubscriptionStatus.PAID) {
    const expected = Number(plan.price);
    const got = Number(amountPaid ?? expected);
    if (got !== expected) throw new HttpError(400, 'INVALID_AMOUNT', `Amount must be ${expected} for plan ${plan.name}`);
  }

  const { business, user, workspace, subscription } = await prisma.$transaction(async (tx: any) => {
    if (paymentReference) {
      const existingRef = await tx.payment.findFirst({ where: { reference: paymentReference } });
      if (existingRef) throw new HttpError(409, 'REFERENCE_EXISTS', 'Payment reference already exists');
    }
...
export async function loginAdmin(input: { phone: string; password: string; rememberMe?: boolean }) {
  const normalizedPhone = normalizeTzPhone(input.phone);
  if (!normalizedPhone) throw new HttpError(400, 'INVALID_PHONE', 'Phone number is invalid');

  const user = await prisma.user.findFirst({
    where: { role: UserRole.ADMIN, phone: normalizedPhone, isActive: true }
  });
  if (!user) throw new HttpError(401, 'INVALID_CREDENTIALS', 'Invalid phone or password');

  const ok = await verifyPassword(input.password, user.passwordHash);
  if (!ok) throw new HttpError(401, 'INVALID_CREDENTIALS', 'Invalid phone or password');
...
export async function loginStaff(input: { employeeId: string; password: string; businessId?: number; rememberMe?: boolean }) {
  const employeeId = String(input.employeeId || '').trim();
  if (!employeeId) throw new HttpError(400, 'INVALID_EMPLOYEE_ID', 'Employee ID is required');
  ...
  if (!matches.length) throw new HttpError(401, 'INVALID_CREDENTIALS', 'Invalid employee ID or password');
  ...
  if (!verified.length) throw new HttpError(401, 'INVALID_CREDENTIALS', 'Invalid employee ID or password');
...
export async function getMe(userId: number) {
  ...
  if (!user || !user.isActive) throw new HttpError(401, 'UNAUTHORIZED', 'Not authenticated');
  ...
  return {
    ...
    workspace: user.workspace
      ...
      : null,
    modules: modules.map((m: any) => ({ key: m.module.key, name: m.module.name, enabled: m.enabled })),
...
export async function requestPasswordReset(input: { phone: string; otp: string }) {
  const phone = normalizeTzPhone(String(input?.phone || '').trim());
  if (!phone) throw new HttpError(400, 'INVALID_PHONE', 'Phone number is invalid');

  const user = await prisma.user.findFirst({ where: { phone, isActive: true } });
  if (!user) throw new HttpError(404, 'PHONE_NOT_FOUND', 'Phone number not found');

  const otp = String(input?.otp || '').replace(/[^0-9]/g, '').slice(0, 6);
  if (otp.length !== 6) throw new HttpError(400, 'INVALID_OTP', 'OTP code is invalid');
...
export async function confirmPasswordReset(input: { phone: string; otp: string; newPassword: string }) {
  const phone = normalizeTzPhone(String(input?.phone || '').trim());
  if (!phone) throw new HttpError(400, 'INVALID_PHONE', 'Phone number is invalid');
  const otp = String(input?.otp || '').replace(/[^0-9]/g, '').slice(0, 6);
  if (otp.length !== 6) throw new HttpError(400, 'INVALID_OTP', 'OTP code is invalid');
  const newPassword = String(input?.newPassword || '');
  if (newPassword.length < 8) throw new HttpError(400, 'INVALID_PASSWORD', 'Password must be at least 8 characters');
  ...
  if (!record || !record.codeHash) throw new HttpError(400, 'OTP_EXPIRED', 'OTP expired. Request a new one');

  const ok = await verifyPassword(otp, record.codeHash);
  if (!ok) {
    await prisma.otp.update({ where: { id: record.id }, data: { attemptCount: record.attemptCount + 1 } });
    throw new HttpError(400, 'INVALID_OTP', 'Invalid code. Try again');
  }
...
async function issueTokens(opts: { userId: number; businessId: number; workspaceId: number | null; role: UserRole; rememberMe?: boolean }) {
  ...
  const jti = randomId();
  const refreshToken = signRefreshToken({ sub: String(opts.userId), jti }, opts.rememberMe ? undefined : 7);
  const tokenHash = hashToken(refreshToken);
...
function summarizeSubscription(subscription: any) {
  ...
  const effectiveStatus = (() => {
    if (rawStatus === SubscriptionStatus.CANCELLED) return SubscriptionStatus.CANCELLED;
    if (endsAtMs && now > endsAtMs) return SubscriptionStatus.EXPIRED;
  ...
  const lockReason = (() => {
    if (!locked) return '';
    if (effectiveStatus === SubscriptionStatus.EXPIRED) return 'expired';
    if (effectiveStatus === SubscriptionStatus.PENDING) return 'payment_pending';
    if (effectiveStatus === SubscriptionStatus.CANCELLED) return 'cancelled';
    return 'inactive';
  })();
